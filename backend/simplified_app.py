"""
Simplified Diet Recommendation System Backend
Only handles user authentication and meal logging
"""
...
import pytz
from simplified_database import SimplifiedDietDatabase
import json
...
def login():
    """Login user with simplified database"""
    try:
...
def register():
    """Register new user with simplified database"""
    try:
        data = request.get_json()
...
def get_profile():
    """Get user profile from simplified database"""
    try:
...
def diet_log():
    """Handle diet logging with simplified database"""
    try:
        token = request.headers.get('Authorization')
        if not token or not token.startswith('Bearer '):
            return jsonify({'error': 'Authorization token required'}), 401
...
def delete_meal(meal_id):
    ...
    try:
        ...
        deleted = db.delete_meal(int(user_id), meal_id)
...
def health_check():
    ...
    return jsonify({
        'status': 'healthy',
        'message': 'Simplified Diet API is running',
        'database': 'SQLite',
        'features': ['User authentication', 'Meal logging'],
        'stats': stats,
...
def home():
    ...
    return jsonify({
        'message': 'Simplified Diet Recommendation System API',
        'version': '3.0.0',
        'status': 'running',
        'database': 'SQLite',
        'features': [
        ...
        'endpoints': {
            ...
            'features': {
                ...
                'foods': 'GET /api/foods/nutrition, GET /api/foods/search'
            },
            'admin': {
...
if __name__ == '__main__':
    print("Starting Simplified Diet Recommendation System...")
    print("Database: SQLite (Users + Meals only)")
